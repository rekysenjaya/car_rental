...
class Histories extends CI_Controller {
  ...
  public function car($id) {
    ...
    if ($method == 'GET') {
      ...
      $resp = $this->Histories->histories_car_data($id, $date->format("Y"), $date->format("m"));
      $car = $this->Histories->histories_car($id);
  ...
  public function client($id) {
    ...
    $method = $_SERVER['REQUEST_METHOD'];
    if ($method == 'GET') {
      $resp = $this->Histories->histories_client_data($id);
      $client = $this->Histories->histories_client($id);
